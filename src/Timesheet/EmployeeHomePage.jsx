...
const EmployeeHomePage = ({ submissions, setSubmissions }) => {
  ...
  useEffect(() => {
    ...
    const fetchSubmissions = async () => {
      try {
        let url = `https://adepuweb20-g4cyfqdre8ftfea5.canadacentral-01.azurewebsites.net/api/timesheets/list/${employeeId}`;
  ...
  const handleDeleteTimesheet = async () => {
    try {
      await axios.delete(`https://adepuweb20-g4cyfqdre8ftfea5.canadacentral-01.azurewebsites.net/api/timesheets/delete/${selectedSubmissionId}`, {
        headers: {
          "Authorization": `Bearer ${token}`
        }
      });
